...
@Test func parsesAllOptions() throws {
    let arguments = ["--name", "Test", "--width", "1280", "--height", "720", "--refresh-rate", "75",
                     ...
                     "--backend", "native", "--streamer-directory", "/tmp/helper"]
    ...
    #expect(value.backend == .native); #expect(value.streamerDevelopmentDirectory == "/tmp/helper")
}
