...
public enum StreamBackend: String, Sendable, Equatable { case streamer, native }

public struct StreamConfiguration: Sendable, Equatable {
    public var name = "PixelFerry Display"
    ...
    public var backend: StreamBackend = .streamer
    public var streamerDevelopmentDirectory: String?
...
public enum CLIParser {
    ...
    Usage: pixelferry [options]
      ...
      --backend <name>       streamer (default) or native
      --streamer-directory <path>  Explicit PixelFerryStreamer source path for development
      --help                 Show this help
    """
    ...
    public static func parse(_ arguments: [String]) throws -> CLIParseResult {
        ...
        while index < arguments.count {
            ...
            case "--streamer-directory": value.streamerDevelopmentDirectory = try next(option); index += 1
        ...
        if let path = value.streamerDevelopmentDirectory, path.isEmpty {
            throw CLIError.invalidValue("--streamer-directory", path)
        }
        return .run(value)
    }
